...
void RFID_task(void *p_arg)
{
	...
	while(1)
	{
		...
		if(RFID_485_STA)
		{
			len=RFID_485_STA&0x3fff;//µÃµ½´Ë´Î½ÓÊÕµ½µÄÊý¾Ý³¤¶È
			for(i=0;i<len;i++)
			{
...
//			USART_RX_STA=0;
		...
		OSTimeDlyHMSM(0,0,0,8,OS_OPT_TIME_HMSM_STRICT,&err); //ÑÓÊ±200ms
	}
}
	...
	//		}
			...
			if(temp<20&&temp>5) 
			{
				...
				if(ult_times2>2)
				{
					...
					OSTimeDlyHMSM(0,0,4,0,OS_OPT_TIME_HMSM_STRICT,&err); //ÑÓÊ±100ms
					Left_BK(0);Right_BK(0);
...
//		printf("AGV_feedback%x\r\n",AGV_feedback);
		...
		if(ult_detection>0) ult_flag=1;//¼ì²âµ½Ç°·½ÓÐÕÏ°­
